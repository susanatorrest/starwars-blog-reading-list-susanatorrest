			...
			<div className="card mb-2" style={{ width: "18rem" }}>
				...
				<div className="card-body bg-white">
					...
					<div className="buttons d-flex justify-content-between mt-3">
						<Link to={"planetsDetails/" + props.index} className="btn btn-outline-primary border-3">
							Learn More!
						</Link>
						<button onClick={() => {!store.favorites.includes(props.name) ? actions.addFavorite(props.name) : ""}} 
        					type="button" className={`btn btn-outline-warning border-3 ${store.favorites.includes(props.name) ? "active" : ""}`}>
